...
class Model(object):
    ...
    def generate_predictions(self, test_coordinates: np.ndarray, test_area_flags: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        adjustment_factor = np.sqrt(2 * np.log(50))
...
def extract_area_information(train_x: np.ndarray, test_x: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    # TODO should I scale the data?

    train_coordinates = train_x[:, :2]  # First two columns are the coordinates
    train_area_flags = train_x[:, 2]  # Third column is the city area information
    test_coordinates = test_x[:, :2]  # First two columns are the coordinates
